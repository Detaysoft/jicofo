               ...
               PacketListener
{
    ...
    {
        // JibriIQ for recording
        // or JibriIQ with 'sipaddress' non-empty for SIP
        return (packet instanceof JibriIq
                && (!isSIP
                        || StringUtils.isNullOrEmpty(
                                ((JibriIq)packet).getSipAddress()))) ||
            // ERROR/RESULT packets coming from our Jibri
    ...
    synchronized public void processPacket(Packet packet)
    {
        ...
        {
            logger.debug(
                "Processing an IQ from " + nickname() + ": " + packet.toXML());
        }
                                        ...
                                        final XmppChatMember    sender)
    {
        ...
        {
            logger.debug(
                nickname() + " request from " + senderMucJid
                    + " iq: " + iq.toXML());
        }
        ...
        if (!verifyModeratorRole(iq))
        {
            logger.warn(
                "Ignored " + nickname()
                    + " request from non-moderator: " + senderMucJid);
            return;
        }
            ...
            currentJibriJid == null)
        {
            ...
            displayName = iq.getDisplayName();
            sipAddress = iq.getSipAddress();
            // Proceed if not empty
            if ((!isSIP && !StringUtils.isNullOrEmpty(streamID))
                || (isSIP && !StringUtils.isNullOrEmpty(sipAddress)))
            {
            ...
            else
            {
                ...
                sendErrorResponse(
                        ...
                        isSIP
                            ? "SIP address is empty or undefined"
        ...
        logger.warn(
            "Discarded: " + iq.toXML() + " - nothing to be done, "
                + recordingOrSIPCall() + " status:" + jibriStatus);
    ...
    private void setJibriStatus(JibriIq.Status newStatus, XMPPError error)
    {
        ...
        {
            SipCallState sipCallStatus = new SipCallState();

            status = sipCallStatus;

            sipCallStatus.setState(newStatus);

            sipCallStatus.setError(error);
        }

        logger.info(
            "Publish new " + nickname()
                + " status: "
                + status.toXML() + " in: " + getRoomName());
        ...
        if (chatRoom2 != null)
        {
            meetTools.sendPresenceExtension(chatRoom2, status);
        }
    }
